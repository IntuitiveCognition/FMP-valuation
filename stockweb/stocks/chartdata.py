...
def get_chart_data(symbol,latest10K_date):
    ...
    try:    
        ...
        final_df = final_df.rename(columns={'revenue':'Revenue', 'rev%':'R%', 'ebitda':'EBITDA', 'ebitda%':'E%', 'roic':'ROIC',
                                 ...
                                 'weightedAverageShsOut':'Shares Out', 'weightedAverageShsOut%':'SO%', 'Stock':'Price', 'Stock%':'Price%'})
    final_df2 = final_df
    final_df2 =final_df2.replace(np.inf, np.nan)
    final_df2 = final_df2.rename_axis('Year').reset_index()
    final_df2 = final_df2.fillna(0)
    chart_json = final_df2.to_dict(orient='records')    
    return chart_json